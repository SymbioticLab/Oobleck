...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="GPU required")
def test_pipeline_1stage_load_microbatch(dummy_pipeline):
    from tests.conftest import TRAIN_BATCH_SIZE

    # one GPU pipeline that handles the entire model
    pipeline: OobleckPipeline = dummy_pipeline(num_gpus_per_pipeline=1, ranks=[0])[0]
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="GPU required")
def test_pipeline_1stage_forward(dummy_pipeline):
    pipeline: OobleckPipeline = dummy_pipeline(num_gpus_per_pipeline=1, ranks=[0])[0]
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="GPU required")
def test_pipeline_1stage_forwardbackward(dummy_pipeline):
    pipeline: OobleckPipeline = dummy_pipeline(num_gpus_per_pipeline=1, ranks=[0])[0]
    ...
    assert all(
        all(
            p._grad is not None if p.requires_grad else None for p in layer.parameters()
        )
        for layer in pipeline.model_layers
    )


# model states checking from: https://discuss.pytorch.org/t/check-if-models-have-same-weights/4351/9
@pytest.mark.skipif(not torch.cuda.is_available(), reason="GPU required")
def test_pipeline_1stage_model_update(dummy_pipeline):
    ...
    assert all(
        ...
        for index, layer in enumerate(pipeline.model_layers)
    )