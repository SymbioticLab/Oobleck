...
class ReconfigurationTestBase(OobleckSingleProcessTestCase):
    class Execution:
        _layers: list[None] = []
    ...
    class FakePipeline:
        def __init__(
            ...
            **kwargs,
            ...
            self._global_step = 0
            self.execution = ReconfigurationTestBase.Execution()
    ...
    class FakeEngine:
        def __init__(
            self,
            test_class: ReconfigurationTestBase,
            sample_args: OobleckArguments,
        ):
    ...
    ) -> ReconfigurationTestBase.FakeEngine:
        mocker.patch(
            "oobleck.planning.instantiator.OobleckPipeline",
            new=ReconfigurationTestBase.FakePipeline,
        )
        mocker.patch(
            "deepspeed.comm.new_group",
            new=ReconfigurationTestBase.FakeProcessGroup,
        )
        mocker.patch(
...
class TestOobleckNoFSDPReconfigurationClass(ReconfigurationTestBase):
    """
    ...
    def test_all_available_templates(
        self,
        fake_engine: ReconfigurationTestBase.FakeEngine,
...
class TestOobleckFSDPReconfigureTestClass(ReconfigurationTestBase):
    """
    ...
    @pytest.mark.parametrize(
        ["num_gpus_fsdp", "failed_ranks", "expected_ranks"],
        [
            (
                2,
                [6, 7],
