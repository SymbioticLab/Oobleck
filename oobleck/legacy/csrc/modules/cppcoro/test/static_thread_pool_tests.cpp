...
TEST_CASE("launch many tasks remotely")
{
	...
	auto makeTask = [&]() -> cppcoro::task<> { co_await threadPool.schedule(); };
...
cppcoro::task<std::uint64_t>
sum_of_squares(std::uint32_t start, std::uint32_t end, cppcoro::static_thread_pool& tp)
{
	...
	if (count > 1000)
	{
		...
		auto [a, b] = co_await cppcoro::when_all(
			sum_of_squares(start, half, tp), sum_of_squares(half, end, tp));
		co_return a + b;
	}
	else
	{
...
TEST_CASE("launch sub-task with many sub-tasks")
{
	...
	std::cout << "multi-threaded version took " << toNs(end - start) << "ns\n"
...
struct fork_join_operation
{
	...
	void begin_work() noexcept { m_count.fetch_add(1, std::memory_order_relaxed); }
...
cppcoro::task<void> for_each_async(SCHEDULER& scheduler, RANGE& range, FUNC func)
{
	...
	struct work_operation
	{
		...
		work_operation(
			fork_join_operation& forkJoin, SCHEDULER& scheduler, FUNC& func, reference_type&& value)
			: m_forkJoin(forkJoin)
			, m_func(func)
	...
	for (auto&& x : range)
	{
		co_await work_operation{ forkJoin, scheduler, func, static_cast<decltype(x)>(x) };
	}
...
TEST_CASE("for_each_async")
{
...

	{
		...
		cppcoro::sync_wait([&]() -> cppcoro::task<> {
			...
			co_await for_each_async(
				tp, values, [](std::uint64_t& value) { value = collatz_distance(value); });

			auto end = std::chrono::high_resolution_clock::now();

			std::cout << "for_each_async of " << values.size() << " took "
...

	{
		...
		std::cout << "single-threaded for loop of " << values.size() << " took "
				  << std::chrono::duration_cast<std::chrono::microseconds>(end - start).count()
				  << "us" << std::endl;
	}
}
