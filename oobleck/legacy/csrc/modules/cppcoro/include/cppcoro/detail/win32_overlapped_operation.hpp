	...
	namespace detail
	{
		...
		public:
			...
			std::size_t get_result()
			{
				if (m_errorCode != 0)
				{
					throw std::system_error{ static_cast<int>(m_errorCode),
											 std::system_category() };
				}
		...
		protected:
			...
			win32_overlapped_operation_cancellable(
				std::uint64_t offset, cancellation_token&& ct) noexcept
				: win32_overlapped_operation_base(
		...
		public:
			...
			bool await_suspend(std::coroutine_handle<> awaitingCoroutine)
			{
				...
				if (canBeCancelled)
				{
					m_cancellationCallback.emplace(std::move(m_cancellationToken), [this] {
						this->on_cancellation_requested();
					});
				}
				else
				{
		...
		private:
				...
				[[maybe_unused]] detail::win32::ulongptr_t completionKey) noexcept
			{
				...
				else
				{
					...
					state =
						operation->m_state.exchange(state::completed, std::memory_order_acq_rel);
					if (state == state::started)
					{
