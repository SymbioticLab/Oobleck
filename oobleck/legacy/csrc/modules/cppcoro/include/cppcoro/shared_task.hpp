...
namespace cppcoro
{
	...
	namespace detail
	{
				...
				void await_suspend(std::coroutine_handle<PROMISE> h) noexcept
				{
					...
					if (waiters != nullptr)
					{
						...
						while (waiter->m_next != nullptr)
						{
							...
							// since resuming the coroutine may destroy the shared_task_waiter
							// value.
		...
		protected:
			bool completed_with_unhandled_exception() { return m_exception != nullptr; }
	...
	namespace detail
	{
		...
		{
			return shared_task<T>{ std::coroutine_handle<shared_task_promise>::from_promise(
				*this) };
		}
		...
		{
			return shared_task<void>{ std::coroutine_handle<shared_task_promise>::from_promise(
				*this) };
		}
	}  // namespace detail

	template<typename AWAITABLE>
	auto make_shared_task(AWAITABLE awaitable) -> shared_task<
