...
namespace cppcoro
{
	...
	public:
		...
		void publish(SEQUENCE sequence) noexcept { m_producerBarrier.publish(sequence); }
		...
		SEQUENCE last_published() const noexcept { return m_producerBarrier.last_published(); }
		...
		///
		/// \return
		/// Returns an Awaitable type that, when awaited, will suspend the awaiting coroutine until
		/// the specified sequence number has been published.
		///
...
#pragma warning(pop)
	...
	public:
		...
		SEQUENCE await_resume() const noexcept { return m_sequencer.m_nextToClaim++; }
	...
	public:
		...
		{
			const SEQUENCE lastAvailableSequence = static_cast<SEQUENCE>(
				m_consumerWaitOperation.await_resume() + m_sequencer.m_bufferSize);
			const SEQUENCE begin = m_sequencer.m_nextToClaim;
			const std::size_t availableCount =
				static_cast<std::size_t>(lastAvailableSequence - begin) + 1;
	...
	template<typename SCHEDULER>
	[[nodiscard]] single_producer_sequencer_claim_operation<SEQUENCE, TRAITS, SCHEDULER>
	single_producer_sequencer<SEQUENCE, TRAITS>::claim_up_to(
		std::size_t count, SCHEDULER& scheduler) noexcept
	{
		return single_producer_sequencer_claim_operation<SEQUENCE, TRAITS, SCHEDULER>(
			*this, count, scheduler);
	}
}  // namespace cppcoro
