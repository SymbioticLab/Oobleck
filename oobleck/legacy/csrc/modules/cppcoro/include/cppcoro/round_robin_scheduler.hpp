...
namespace cppcoro
{
	...
	{
		static_assert(
			N >= 2, "Round robin scheduler must be configured to support at least two coroutines");
	...
	public:
		...
		~round_robin_scheduler()
		{
			...
			assert(std::all_of(
				m_coroutines.begin(), m_coroutines.end(), [&](auto h) { return h == m_noop; }));
		}
	...
	private:
		std::coroutine_handle exchange_next(std::coroutine_handle<> coroutine) noexcept
		{
			auto coroutineToResume =
				std::exchange(m_scheduler.m_coroutines[m_scheduler.m_index], awaitingCoroutine);
