...
namespace cppcoro
{
	...
	{
		static_assert(
			std::is_integral_v<SEQUENCE>, "sequence_barrier requires an integral sequence type");
...
#pragma warning(pop)
	...
	protected:
		...
		{
			// This synchronises with the exchange(true, std::memory_order_acquire) in
			// await_suspend().
	...
	template<typename SCHEDULER>
	[[nodiscard]] sequence_barrier_wait_operation<SEQUENCE, TRAITS, SCHEDULER>
	sequence_barrier<SEQUENCE, TRAITS>::wait_until_published(
		SEQUENCE targetSequence, SCHEDULER& scheduler) const noexcept
	{
		return sequence_barrier_wait_operation<SEQUENCE, TRAITS, SCHEDULER>(
			*this, targetSequence, scheduler);
	}
	...
	void sequence_barrier<SEQUENCE, TRAITS>::add_awaiter(awaiter_t* awaiter) const noexcept
	{
		...
		do
		{
			...
			// published sequence number. The producer thread may not have seen our write to
			// m_awaiters so we need to try to re-acquire the list of awaiters to ensure that the
			// waiters that are now satisfied are woken up.
			...
			{
				const auto diff =
					TRAITS::difference(awaiters->m_targetSequence, lastKnownPublished);
				if (diff > 0)
				{
					*awaitersToRequeueTail = awaiters;
					awaitersToRequeueTail = &awaiters->m_next;
