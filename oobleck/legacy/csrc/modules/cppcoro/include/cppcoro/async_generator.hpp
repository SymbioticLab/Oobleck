...
namespace cppcoro
{
	...
	namespace detail
	{
		...
		public:
			...
			async_generator_iterator<T> await_resume()
			{
				...
				return async_generator_iterator<T>{ handle_type::from_promise(
	...
	namespace detail
	{
		...
		public:
			...
			bool request_cancellation() noexcept
			{
				const auto previousState =
					m_state.exchange(state::cancelled, std::memory_order_acq_rel);

				// Not valid to destroy async_generator<T> object if consumer coroutine still
				// suspended in a co_await for next item.
		...
		public:
			async_generator_yield_operation(
				async_generator_promise_base& promise, state initialState) noexcept
				: m_promise(promise)
		...
		inline bool
		async_generator_yield_operation::await_suspend(std::coroutine_handle<> producer) noexcept
		{
			...
			if (currentState == state::value_not_ready_consumer_active)
			{
				...
				if (currentState == state::value_not_ready_consumer_suspended)
				{
					...
					m_promise.m_state.store(
						state::value_ready_producer_active, std::memory_order_relaxed);

					m_promise.m_consumerCoroutine.resume();

					// The consumer might have asked for another value before returning, in which
					// case it'll transition to value_not_ready_consumer_suspended and we can return
					// without suspending, otherwise we should try to suspend the producer, in which
					// case the consumer will wake us up again when it wants the next value.
					//
					// Need to use acquire semantics here since it's possible that the consumer
					// might have asked for the next value on a different thread which executed
					// concurrently with the call to m_consumerCoro.resume() above.
			...
			if (currentState == state::value_ready_producer_active)
			{
				...
				// If we failed to suspend then it's either because the consumer destructed,
				// transitioning the state to cancelled, or requested the next item, transitioning
				// the state to value_not_ready_consumer_suspended.
		...
		protected:
				...
				, m_producerCoroutine(producerCoroutine)
			{
				...
				if (initialState == state::value_ready_producer_suspended)
				{
					...
					// on the same thread.
					promise.m_state.store(
						state::value_not_ready_consumer_active, std::memory_order_relaxed);
		...
		public:
			...
			bool await_suspend(std::coroutine_handle<> consumerCoroutine) noexcept
			{
				...
				// value_not_ready_consumer_suspended. This could be racing with producer making the
				// next value available and suspending (transition to
				// value_ready_producer_suspended) so we use compare_exchange to decide who wins the
				// race. If compare_exchange succeeds then consumer suspended (and we return true).
	...
	async_generator<std::invoke_result_t<
		FUNC&,
		decltype(*std::declval<typename async_generator<T>::iterator&>())>>
