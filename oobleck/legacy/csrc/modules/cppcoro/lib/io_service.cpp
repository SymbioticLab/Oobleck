...
public:
	...
	void dequeue_due_timers(
		time_point currentTime, cppcoro::io_service::timed_schedule_operation*& timerList) noexcept;

	void
	remove_cancelled_timers(cppcoro::io_service::timed_schedule_operation*& timerList) noexcept;
	...
	cppcoro::io_service::timed_schedule_operation*& timerList) noexcept
{
	...
	auto firstCancelledEntry =
		std::find_if(m_timerEntries.begin(), m_timerEntries.end(), isTimerCancelled);
...
bool cppcoro::io_service::try_process_one_event(bool waitForEvent)
{
	...
	while (true)
	{
		...
		BOOL ok = ::GetQueuedCompletionStatus(
			m_iocpHandle.handle(), &numberOfBytesTransferred, &completionKey, &overlapped, timeout);
		...
		else if (ok)
		{
			if (completionKey != 0)
			{
				...
				// io_service::schedule().
				std::coroutine_handle<>::from_address(reinterpret_cast<void*>(completionKey))
					.resume();
				return true;
			}
...
void cppcoro::io_service::timed_schedule_operation::await_suspend(std::coroutine_handle<> awaiter)
{
	...
	if (m_cancellationToken.can_be_cancelled())
	{
		m_cancellationRegistration.emplace(
			m_cancellationToken, [timerState] { timerState->request_timer_cancellation(); });
	}
	...
	do
	{
		...
		prev, this, std::memory_order_release, std::memory_order_acquire));
