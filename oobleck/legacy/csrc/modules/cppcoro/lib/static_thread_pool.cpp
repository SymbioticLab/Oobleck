...
#include <chrono>
#include <mutex>
#include <utility>  // GCC12 requires it to use std::exchange
...
namespace cppcoro
{
	...
	public:
		explicit thread_state()
			: m_localQueue(std::make_unique<std::atomic<schedule_operation*>[]>(
		...
		bool try_local_enqueue(schedule_operation*& operation) noexcept
		{
			...
			std::unique_ptr<std::atomic<schedule_operation*>[]> newLocalQueue{ new (
				std::nothrow) std::atomic<schedule_operation*>[newSize] };
			if (!newLocalQueue)
			{
...
#pragma warning(pop)
	...
	bool
	static_thread_pool::approx_has_any_queued_work_for(std::uint32_t threadIndex) const noexcept
	{
	...
	static_thread_pool::try_steal_from_other_thread(std::uint32_t thisThreadIndex) noexcept
	{
		...
		for (std::uint32_t otherThreadIndex = 0; otherThreadIndex < m_threadCount;
			 ++otherThreadIndex)
		{
