...
void cppcoro::async_auto_reset_event::reset() noexcept
{
	...
	while (local::get_set_count(oldState) > local::get_waiter_count(oldState))
	{
		if (m_state.compare_exchange_weak(
				oldState, oldState - local::set_increment, std::memory_order_relaxed))
		{
			// Successfully reset.
			return;
		}
...
void cppcoro::async_auto_reset_event::resume_waiters(std::uint64_t initialState) const noexcept
{
	...
	do
	{
		...
		const std::uint64_t delta =
			std::uint64_t(waiterCountToResume) | std::uint64_t(waiterCountToResume) << 32;
		...
		const std::uint64_t newState = m_state.fetch_sub(delta, std::memory_order_acq_rel) - delta;
