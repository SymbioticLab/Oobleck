...
class Simulator:
    ...
    def __init__(self,
        ...
        logger.info(f'Simulating: oobleck')
        ...
        # Here when failure happens
        # I call local_rendezvous(), this restart overhead is added to local_rendezvous() time for each model 
        ...
        if model == 'BERT-large':
            ...
            self.local_rendezvous_timeout_delta = 2_000
            ...
            self.local_rendezvous_timeout_delta = 3_000
            self.num_stages_target = 12
            self.on_demand_num_instances = 4 * 8
            self.on_demand_cost = self.on_demand_num_instances * self.on_demand_cost_per_hour
            self.on_demand_performance = self.samples_per_step / 3.37
            ...
            self.local_rendezvous_timeout_delta = 6_000
            self.num_stages_target = 12
            self.on_demand_num_instances = 4 * 8
            self.on_demand_cost = self.on_demand_num_instances * self.on_demand_cost_per_hour
            self.on_demand_performance = self.samples_per_step / 3.37
            ...
            self.local_rendezvous_timeout_delta = 10_000
            self.num_stages_target = 12
            self.on_demand_num_instances = 4 * 8
            self.on_demand_cost = self.on_demand_num_instances * self.on_demand_cost_per_hour
            self.on_demand_performance = self.samples_per_step / 3.37
    ...
    def create_global_rendezvous_timeout_event(self, delta):
        self.rebalance_time += (self.global_rendezvous_timeout_delta+self.local_rendezvous_timeout_delta)
    ...
    def simulate_spot_instance_remove(self, delta, data):
        ...
        # check if step complete event is in the queue
        heapq.heapify(self.events)
        for event in self.events:
            if event.kind == EventKind.TRAINING_STEP_COMPLETE:
                self.step_wasted = True
                break
    ...
    def simulate_training_step_complete(self, delta, data):
        ...
        if self.step_wasted:
            self.step_wasted = False
            # self.num_steps_complete -= 1
            # self.performance_ys[-1] = 0
            # self.performance_ys[-2] = 0
