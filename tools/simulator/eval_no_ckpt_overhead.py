...
def run_simulator_on_traces(model, bar_trace_list):
    bar_result_dict = {'Varuna (no ckpt)':{}, 'varuna':{}, 'oobleck':{}}
    for trace_path in tqdm(bar_trace_list):
        ...
        simulator = varuna_simulator(
            ...
            time_hard_stop=True, no_ckpt_overhead=True, checkpoint_per_iter=2
        )
        result = simulator.simulate()
        bar_result_dict['Varuna (no ckpt)'][trace_path] = result.average_performance
        csv_file.close()
...
def plot_results(result_dict, model_name, save_path=None, show=False):
    bamboo_oom = False
    if list(result_dict['Varuna (no ckpt)'].values())[0]==0:
        bamboo_oom = True
    ...
    for i, (x, k) in enumerate(zip(X, bar_results_data.keys())):
        ...
        if i==0:
            # get label one time
            plt.bar(x-barWidth, data_dict['varuna'], color=VARUNA_COLOR, width=barWidth, label="Varuna", alpha=ALPHA)
            plt.bar(x, data_dict['Varuna (no ckpt)'], color=VARUNA_NOCKPT_COLOR, width=barWidth, label="Varuna (no ckpt)", alpha=ALPHA)
            plt.bar(x+barWidth, data_dict['oobleck'], color=OOBLECK_COLOR, width=barWidth, label="Oobleck", alpha=ALPHA)
        else:
            plt.bar(x-barWidth, data_dict['varuna'], color=VARUNA_COLOR, width=barWidth, alpha=ALPHA)
            plt.bar(x, data_dict['Varuna (no ckpt)'], color=VARUNA_NOCKPT_COLOR, width=barWidth, alpha=ALPHA)
