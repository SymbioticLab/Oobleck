...
class Simulator:
    ...
    def __init__(self,
                 ...
                 varuna=False, 
                 steps_per_run=None, 
                 time_hard_stop=False, detail_res=False):
        ...
        # 6h
        # self.redundant_ratio = 1 - 0.2785
        # 10m
        self.redundant_ratio = 1 - 0.1971

        logger.info(f'Simulating: bamboo')
        ...
        if model == 'BERT-large':
            ...
            self.local_rendezvous_timeout_delta = 80000
            ...
            self.local_rendezvous_timeout_delta = 100_000
            self.num_stages_target = 12
            self.on_demand_num_instances = 4 * 8
            self.on_demand_cost = self.on_demand_num_instances * self.on_demand_cost_per_hour
            self.on_demand_performance = self.samples_per_step / 3.37
            ...
            self.local_rendezvous_timeout_delta = 230_000
            self.num_stages_target = 12
            self.on_demand_num_instances = 4 * 8
            self.on_demand_cost = self.on_demand_num_instances * self.on_demand_cost_per_hour
            self.on_demand_performance = self.samples_per_step / 3.37
            ...
            self.local_rendezvous_timeout_delta = 450_000
            self.num_stages_target = 12
            self.on_demand_num_instances = 4 * 8
            self.on_demand_cost = self.on_demand_num_instances * self.on_demand_cost_per_hour
            self.on_demand_performance = self.samples_per_step / 3.37
    ...
    def create_global_rendezvous_timeout_event(self, delta):
        self.restart_time += self.global_rendezvous_timeout_delta
    ...
    def create_local_rendezvous_timeout_event(self, delta):
        self.rebalance_time += self.local_rendezvous_timeout_delta
        return self.create_event(
    ...
    def create_training_step_complete_event(self, delta, rendezvous_version):
        self.effective_time += (self.step_delta * (1-self.redundant_ratio) ) 
        self.redundant_time += (self.step_delta * self.redundant_ratio)
        return self.create_event(
    ...
    def simulate_spot_instance_remove(self, delta, data):        
        ...
        try:
            self.fallback_loss_time += self.step_delta
        except:
            self.fallback_loss_time += 0

        if instance.is_running():
            # This is a fatal failure
            if len(instance.active_coordinates) > 1 or self.varuna:
    ...
    def simulate(self, duration=None):
        ...
        detailed_result['instances'] = [instances_xs, instances_ys, result.average_instances]
        detailed_result['performance'] = [self.performance_xs, self.performance_ys, result.average_performance]
        ...
        print("Fallback:{}, {}".format(self.fallback_loss_time, self.fallback_loss_time/total))
